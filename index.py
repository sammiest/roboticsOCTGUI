...
#from RAOCTg2.util.node import launch, watch
...
from igmr_robotics_toolkit.node import launch, watch
...
class Ui_IndexWindow(object):
    ...
    def initialize_robot_state(self,config_file_path):
        with open(config_file_path, 'r') as file:
            config = yaml.safe_load(file)

    
        robot_state = RobotState(
        robot_host=config.get('robot_host', '10.162.76.28'),  
        robot_id=config.get('robot_id', 'default_id'),
        host_sequence=config.get('host_sequence', 0),
        robot_sequence=config.get('robot_sequence', 0),
        host_timestamp=config.get('host_timestamp', 0),
        robot_timestamp=config.get('robot_timestamp', 0),
        actual_q=config.get('actual_q', [0, 0, 0]),
        actual_qd=config.get('actual_qd', [0, 0, 0]),
        actual_qdd=config.get('actual_qdd', [0, 0, 0]),
        target_q=config.get('target_q', [0, 0, 0]),
        target_qd=config.get('target_qd', [0, 0, 0]),
        target_qdd=config.get('target_qdd', [0, 0, 0]),
        actual_ee=config.get('actual_ee', [0, 0, 0]),
        actual_ee_twist=config.get('actual_ee_twist', [0, 0, 0]),
        actual_ee_wrench=config.get('actual_ee_wrench', [0, 0, 0]),
        target_ee=config.get('target_ee', [0, 0, 0]),
        target_ee_twist=config.get('target_ee_twist', [0, 0, 0]),
        target_ee_wrench=config.get('target_ee_wrench', [0, 0, 0]),
        actual_torque=config.get('actual_torque', [0, 0, 0]),
        target_torque=config.get('target_torque', [0, 0, 0]),
        external_torque=config.get('external_torque', [0, 0, 0]),
        external_ee_wrench=config.get('external_ee_wrench', [0, 0, 0]),
    ...
    def robot_action(self, state):
        ...
        self._publisher = Publisher(self._domain_participant)
        ...
        #self._robot_state = SubscribedStateBuffer(self._params.get('robot/topic'), RobotState, expiration=lambda s: s.timestamp + 0.1, domain_participant=self._domain_participant)
            _robot_state = SubscribedStateBuffer(self._params.get('topic/robot_state'), RobotState)
...
#                                        robotstate.external_torque,  robotstate.external_ee_wrench) #help
            ...
            case 'jog':
                ...
                print('jog')
            case 'watch':
                self._robot_coordinator.change_strategy('watch')
                print('watch')
            case 'face_tracking':
                self._robot_coordinator.change_strategy('face_tracking')
                print('face tracking')
            case 'pupil_tracking':
                self._robot_coordinator.change_strategy('pupil_tracking')
                print('pupil tracking')
            case 'retreat':
                self._robot_coordinator.change_strategy('retreat')
